...
>
    ...
    <div
        class="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl shadow-xl bottom-sheet max-w-2xl mx-auto flex flex-col max-h-[85vh]"
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar px-4 sm:px-6 py-3 sm:py-4">
            <template x-if="$store.cart.items.length === 0">
                <div class="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <button @click="$store.ui.cartOpen = false" class="px-5 py-2 bg-primary text-white rounded-full text-sm font-semibold active:scale-95 transition-transform shadow-md shadow-primary/20">
                        Browse Menu
                    </button>
                </div>
            </template>
            ...
            <div class="shrink-0 sticky bottom-0 bg-white/95 backdrop-blur-md border-t border-gray-100 px-4 sm:px-6 py-3 sm:py-4">
                ...
                <a href="{{ route('checkout') }}" class="gradient-button w-full h-11 sm:h-12 rounded-full flex items-center justify-between px-4 sm:px-5 text-white text-xs sm:text-sm font-semibold active:scale-[0.98] transition-all shadow-lg shadow-primary/20">
                    <span class="flex items-center gap-2">
                        <span class="material-symbols-outlined text-base sm:text-lg">payments</span>
                        Proceed to Checkout
                    </span>
...
>
    ...
    <div
        class="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl shadow-xl bottom-sheet max-w-2xl mx-auto flex flex-col max-h-[90vh]"
        x-transition:enter="transition-transform duration-500 ease-out"
        x-transition:enter-start="translate-y-full"
        x-transition:enter-end="translate-y-0"
        x-transition:leave="transition-transform duration-500 ease-in"
    ...
    >
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar">
            <div class="shrink-0 relative aspect-video w-full bg-gradient-to-br from-primary/5 to-secondary/5 overflow-hidden">
                <img
                    :src="$store.ui.detailItem?.image || '{{ asset('assets/images/default/no-image.svg') }}'"
                    :alt="$store.ui.detailItem?.name"
            ...
            <div class="px-4 sm:px-6 pt-3 sm:pt-4 pb-2">
                ...
                <p class="text-sm text-gray-500 leading-relaxed">
                    Indulge in our signature bite-sized masterpiece. A hand-pressed patty, infused with premium ingredients and served with artisanal touches.
                </p>
            </div>
            ...
            <div class="px-4 sm:px-6 py-4 border-t border-gray-100">
                ...
                <div class="flex gap-2">
                    <template x-for="level in ['Low', 'Medium', 'High']">
                        <button
                            @click="$store.ui.setSpicy(level)"
                            :class="$store.ui.spicyLevel === level ? 'border-primary bg-primary/5 text-primary' : 'border-gray-200 text-gray-500 hover:bg-gray-50'"
            ...
            <div class="px-4 sm:px-6 py-4 border-t border-gray-100">
                ...
                <textarea
                    x-model="$store.ui.note"
                    class="w-full bg-gray-50 border-none rounded-xl px-4 py-3 text-sm text-gray-700 placeholder:text-gray-300 focus:ring-2 focus:ring-primary/20 resize-none h-24"
...
<div
    ...
    x-cloak
    class="fixed bottom-[70px] left-3 right-3 bg-gray-900 text-white py-3 px-4 rounded-2xl shadow-xl flex justify-between items-center z-40 max-w-lg mx-auto"
