    ...
    <div class="flex items-center justify-between w-full max-w-4xl mx-auto">
        ...
        <div class="flex items-center gap-2 sm:gap-3">
            ...
            <button
                ...
                class="relative w-10 h-10 flex items-center justify-center rounded-full hover:bg-gray-100 transition-colors active:scale-90"
            >
                <span class="material-symbols-outlined text-gray-600">shopping_cart</span>
                <template x-if="$store.cart.count > 0">
                    <span class="absolute -top-0.5 -right-0.5 bg-secondary text-white text-[10px] font-bold min-w-[18px] h-[18px] flex items-center justify-center rounded-full border-2 border-white px-0.5" x-text="$store.cart.count"></span>
                </template>
            </button>
        </div>
    </div>
