...
@section('title', 'Checkout - Verdant Bistro')
...
<div class="max-w-2xl mx-auto">
    ...
    <section class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-6">
        ...
        <div class="px-5 py-3 bg-gray-50/50 space-y-1.5">
            <div class="flex justify-between items-center text-sm">
                ...
                <span class="text-gray-800" x-text="'Rp ' + $store.cart.subtotal.toLocaleString('id-ID')"></span>
            </div>
            <div class="flex justify-between items-center text-sm">
                <span class="text-gray-500">Tax &amp; Service (15%)</span>
                <span class="text-gray-800" x-text="'Rp ' + ($store.cart.serviceFee + $store.cart.tax).toLocaleString('id-ID')"></span>
            </div>
        </div>
    </section>
    ...
    <section class="mb-8">
        ...
        <div class="space-y-3">
            <button class="w-full px-5 py-4 bg-white border-2 border-primary rounded-xl flex items-center justify-between shadow-sm active:scale-[0.99] transition-all">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-full bg-primary/10 text-primary flex items-center justify-center">
                        <span class="material-symbols-outlined">payments</span>
                    </div>
                    <div class="text-left">
                        <p class="text-sm font-semibold text-gray-800">Pay at Cashier</p>
                        <p class="text-xs text-gray-400">Pay after your meal</p>
                    </div>
                </div>
            ...
            <div class="w-full px-5 py-4 bg-gray-50/50 border border-gray-100 rounded-xl flex items-center justify-between opacity-60 cursor-not-allowed">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-full bg-gray-100 text-gray-300 flex items-center justify-center">
                        <span class="material-symbols-outlined">qr_code_2</span>
                    </div>
                    <div class="text-left">
                        <p class="text-sm font-semibold text-gray-400">QRIS / Digital</p>
                        <p class="text-xs text-gray-300">Fast &amp; secure payment</p>
                    </div>
                </div>
                <span class="bg-secondary/10 text-secondary text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-tight">Coming Soon</span>
            </div>
        </div>
    </section>
</div>
    ...
    <div class="max-w-2xl mx-auto flex flex-col gap-3">
        <div class="flex justify-between items-end">
            <div>
                ...
                <p class="text-lg sm:text-xl font-bold text-primary" x-text="'Rp ' + $store.cart.total.toLocaleString('id-ID')"></p>
            </div>
            <span class="text-xs text-gray-400 italic">Ref: #VB-9912</span>
        </div>
        <form method="POST" action="{{ route('checkout.store') }}" class="w-full">
            @csrf
            <button type="submit" class="w-full h-11 sm:h-12 rounded-full bg-primary text-white font-semibold text-xs sm:text-sm flex items-center justify-center gap-2 shadow-lg active:scale-[0.98] transition-transform animate-pulse-emerald">
