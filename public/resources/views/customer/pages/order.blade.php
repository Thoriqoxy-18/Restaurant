    ...
    <section class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 mb-6 relative overflow-hidden">
        <div class="flex justify-between items-start relative z-10">
            ...
            @foreach ($stepLabels as $i => $label)
                <div class="flex flex-col items-center gap-1.5 w-1/4">
                    ...
                    @elseif ($i == $currentStep)
                        <div class="relative flex items-center justify-center w-10 h-10">
                            ...
                            <div class="w-10 h-10 rounded-full bg-primary flex items-center justify-center text-white shadow-lg animate-pulse-dot">
                ...
                @if ($i < count($stepLabels) - 1)
                    <div class="absolute top-5 left-{{ ($i * 25 + 12.5) }}% right-{{ (75 - $i * 25) }}% h-0.5 {{ $i < $currentStep ? 'bg-primary' : 'bg-gray-200' }}" style="left: {{ $i * 25 + 12.5 }}%; right: {{ 75 - $i * 25 }}%"></div>
                @endif
            @endforeach
        </div>
        ...
        <div class="mt-6 pt-4 border-t border-gray-100 flex items-center justify-between">
            <div class="flex items-center gap-3">
                <div class="w-11 h-11 rounded-full bg-tertiary/10 flex items-center justify-center">
                    <span class="material-symbols-outlined text-tertiary">timer</span>
                </div>
                <div>
                    <p class="text-xs text-gray-400">Estimated Arrival</p>
                    <p class="text-sm font-semibold text-gray-800">15-20 mins</p>
                </div>
            </div>
            <div class="text-right">
                <p class="text-xs text-gray-400">Order Time</p>
                <p class="text-sm font-semibold text-primary">{{ $order->created_at->format('h:i A') }}</p>
            </div>
        </div>
    </section>
    ...
    <section class="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm mb-6">
        <div class="space-y-2">
            ...
            <div class="flex justify-between text-sm text-gray-500">
                ...
                <span>Rp {{ number_format($order->service_fee + $order->tax, 0, ',', '.') }}</span>
            </div>
