    ...
    <div class="grid grid-cols-1 md:grid-cols-12 gap-6 mb-8">
        ...
        <div class="md:col-span-4 bg-primary text-on-primary rounded-xl p-6 flex flex-col items-center justify-center text-center">
            ...
            <form method="POST" action="{{ route('admin.tables.store') }}" id="add-table-form" class="hidden w-full mt-3 space-y-2">
                @csrf
                <input name="name" required placeholder="Nama meja (mis. Meja 11)" class="w-full px-3 py-2 rounded-lg bg-on-primary text-primary placeholder:text-primary/60 font-body-md text-body-md border border-on-primary/30"/>
                <input name="capacity" type="number" min="1" value="4" class="w-full px-3 py-2 rounded-lg bg-on-primary text-primary font-body-md text-body-md border border-on-primary/30"/>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        <div class="bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden flex flex-col">
            ...
            <div class="p-6 flex flex-col items-center justify-center flex-1 bg-surface-bright">
                <div class="w-32 h-32 bg-white border border-outline-variant p-2 rounded-lg mb-4 flex items-center justify-center demo-qr">
                    {!! \App\Support\DemoQrCode::svg(url('/menu?table=' . $table->id)) !!}
                </div>
                <p class="font-label-sm text-label-sm text-on-surface-variant break-all text-center">/menu?table={{ $table->id }}</p>
            </div>
            <div class="p-4 border-t border-surface-variant flex gap-2">
                <a href="{{ url('/menu?table=' . $table->id) }}" target="_blank" class="flex-1 py-2 border border-secondary text-secondary rounded font-label-sm text-label-sm hover:bg-secondary hover:text-on-secondary transition-colors text-center flex items-center justify-center gap-1">
