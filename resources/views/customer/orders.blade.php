...
<div class="max-w-2xl mx-auto">
    ...
    @if ($orders->isEmpty())
        <div class="flex flex-col items-center justify-center py-16 text-center">
            ...
            <a href="{{ route('home') }}" class="px-5 py-2 bg-primary text-white rounded-full text-sm font-semibold active:scale-95 transition-transform shadow-md shadow-primary/20">Browse Menu</a>
        </div>
    @else
        <div class="space-y-3">
            @foreach ($orders as $order)
                <a href="{{ route('customer.order-detail', $order->id) }}" class="block bg-white rounded-xl border border-gray-100 shadow-sm p-4 hover:shadow-md transition-shadow">
                    ...
                    <div class="mt-1.5 text-xs text-gray-400">{{ $order->orderItems->count() }} item(s) &middot; Table {{ $order->restaurantTable->table_number ?? '-' }}</div>
                </a>
            @endforeach
        </div>
    @endif
