...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden max-w-2xl mx-auto">
    <div class="relative aspect-[4/3] bg-gradient-to-br from-primary/5 to-secondary/5 overflow-hidden">
        <img src="{{ $menuItem->image ? asset($menuItem->image) : asset('assets/images/default/no-image.svg') }}" alt="{{ $menuItem->name }}" class="w-full h-full object-cover" loading="lazy">
    ...
    <div class="px-4 sm:px-6 py-5">
        ...
        <p class="text-sm text-gray-500 leading-relaxed mb-6">{{ $menuItem->description }}</p>
        ...
        <div class="py-4 border-t border-gray-100">
            ...
            <div class="space-y-2">
                <label class="flex items-center justify-between px-4 py-3 rounded-xl hover:bg-gray-50 transition-colors cursor-pointer border border-transparent">
                    <div class="flex items-center gap-3"><input type="checkbox" class="w-4 h-4 rounded border-gray-300 text-primary focus:ring-primary/30"><span class="text-sm text-gray-700">Caramelized Onion</span></div>
                    <span class="text-xs font-medium text-gray-400">+Rp 5.000</span>
                </label>
                <label class="flex items-center justify-between px-4 py-3 rounded-xl hover:bg-gray-50 transition-colors cursor-pointer border border-transparent">
                    <div class="flex items-center gap-3"><input type="checkbox" class="w-4 h-4 rounded border-gray-300 text-primary focus:ring-primary/30"><span class="text-sm text-gray-700">Extra Cheese</span></div>
        ...
        <div class="pt-4 border-t border-gray-100 flex items-center gap-4">
            <div class="flex items-center bg-gray-100 rounded-full p-0.5">
                <button class="w-10 h-10 flex items-center justify-center rounded-full bg-white shadow-sm text-gray-600 active:scale-90 transition-transform"><span class="material-symbols-outlined text-lg">remove</span></button>
                <span class="w-10 text-center font-semibold text-gray-800">1</span>
                <button class="w-10 h-10 flex items-center justify-center rounded-full bg-white shadow-sm text-gray-600 active:scale-90 transition-transform"><span class="material-symbols-outlined text-lg">add</span></button>
            </div>
            <button class="flex-1 h-12 rounded-full gradient-button text-white font-semibold text-sm flex items-center justify-center gap-2 active:scale-[0.98] transition-all shadow-lg shadow-primary/20">
                Add to Cart <span class="material-symbols-outlined text-lg">shopping_basket</span>
            </button>
        </div>
    </div>
</div>
