...
<div class="max-w-2xl mx-auto">
    ...
    <template x-if="$store.cart.items.length === 0">
        <div class="flex flex-col items-center justify-center py-16 text-center">
            ...
            <a href="{{ route('home') }}" class="px-6 py-2.5 bg-primary text-white rounded-full text-sm font-semibold active:scale-95 transition-transform shadow-md shadow-primary/20">Browse Menu</a>
        </div>
    </template>
    <template x-if="$store.cart.items.length > 0">
        <section class="mt-8 p-6 bg-white rounded-2xl border border-gray-100 shadow-sm">
            <div class="space-y-3">
                ...
                <div class="flex justify-between text-sm text-gray-500"><span>Service Fee (5%)</span><span x-text="'Rp ' + $store.cart.serviceFee.toLocaleString('id-ID')"></span></div>
                <div class="flex justify-between text-sm text-gray-500"><span>Tax (10%)</span><span x-text="'Rp ' + $store.cart.tax.toLocaleString('id-ID')"></span></div>
                <div class="h-px bg-gray-100"></div>
                <div class="flex justify-between text-base font-semibold text-gray-800"><span>Total</span><span class="text-primary" x-text="'Rp ' + $store.cart.total.toLocaleString('id-ID')"></span></div>
            </div>
        </section>
    </template>
    <template x-if="$store.cart.items.length > 0">
        <div class="mt-6">
            <a href="{{ route('checkout') }}" class="gradient-button w-full h-11 sm:h-12 rounded-full flex items-center justify-between px-4 sm:px-6 text-white font-semibold text-xs sm:text-sm active:scale-[0.98] transition-all shadow-lg shadow-primary/20">
                <span class="flex items-center gap-2"><span class="material-symbols-outlined text-lg">payments</span> Proceed to Checkout</span>
