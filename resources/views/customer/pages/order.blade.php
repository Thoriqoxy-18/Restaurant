...
@section('title', 'Order #' . $order->order_number . ' - Verdant Bistro')
@section('content')
<div class="max-w-2xl mx-auto">
    ...
    <section class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 mb-6 relative overflow-hidden">
        <div class="flex justify-between items-start relative z-10">
            @php $steps = ['pending' => 0, 'processing' => 1, 'completed' => 2, 'cancelled' => 2]; $currentStep = $steps[$order->status] ?? 0; $stepLabels = ['Waiting', 'Processing', 'Delivery', 'Served']; $stepIcons = ['check', 'restaurant', 'delivery_dining', 'flatware']; @endphp
            @foreach ($stepLabels as $i => $label)
                <div class="flex flex-col items-center gap-1.5 w-1/4">
                    @if ($i < $currentStep)
                        <div class="w-10 h-10 rounded-full bg-primary flex items-center justify-center text-white shadow-sm"><span class="material-symbols-outlined text-lg" style="font-variation-settings: 'FILL' 1;">{{ $stepIcons[$i] }}</span></div>
                        <span class="text-[11px] font-medium text-primary text-center">{{ $label }}</span>
                    @elseif ($i == $currentStep)
                        <div class="relative flex items-center justify-center w-10 h-10">
                            <div class="absolute w-full h-full bg-primary/20 rounded-full animate-pulse-ring"></div>
                            <div class="w-10 h-10 rounded-full bg-primary flex items-center justify-center text-white shadow-lg animate-pulse-dot"><span class="material-symbols-outlined text-lg">{{ $stepIcons[$i] }}</span></div>
                        </div>
                        <span class="text-[11px] font-bold text-primary text-center">{{ $label }}</span>
                    @else
                        <div class="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-gray-300"><span class="material-symbols-outlined text-lg">{{ $stepIcons[$i] }}</span></div>
