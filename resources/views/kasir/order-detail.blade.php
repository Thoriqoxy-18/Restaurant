...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <a href="{{ route('kasir.orders') }}" class="inline-flex items-center gap-1 text-sm text-gray-400 hover:text-gray-600 mb-1">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="space-y-4">
            ...
            <section class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                ...
                <div class="space-y-2">
                    <form method="POST" action="{{ route('kasir.orders.update-status', $order->id) }}">
                        @csrf @method('PATCH')
                        <select name="status" class="w-full h-10 rounded-xl border border-gray-200 text-sm px-3 mb-2 focus:border-primary focus:ring-2 focus:ring-primary/10 outline-none">
                            <option value="pending" {{ $order->status == 'pending' ? 'selected' : '' }}>Pending</option>
                            <option value="processing" {{ $order->status == 'processing' ? 'selected' : '' }}>Processing</option>
                            <option value="completed" {{ $order->status == 'completed' ? 'selected' : '' }}>Completed</option>
                            <option value="cancelled" {{ $order->status == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                        </select>
                        <button type="submit" class="w-full h-10 rounded-xl bg-primary text-white text-sm font-semibold hover:bg-primary/90 transition-colors">Update Status</button>
                    </form>
                </div>
            </section>
            ...
            <section class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                ...
                <form method="POST" action="{{ route('kasir.orders.payment', $order->id) }}" class="space-y-3">
                    @csrf @method('PATCH')
                    <div>
                    ...
                    <button type="submit" class="w-full h-10 rounded-xl bg-green-500 text-white text-sm font-semibold hover:bg-green-600 transition-colors flex items-center justify-center gap-1">
